...
export const Recorder: React.FC<RecorderProps> = ({ onRecordingComplete }) => {
  ...
  return (
    <div className="flex flex-col items-center space-y-4 w-full">
      ...
      <div className={`relative transition-all duration-300 ${isRecording ? 'scale-110' : ''}`}>
        <Button 
          onClick={handleToggleRecording} 
          disabled={!!error || isProcessing}
          className={`${isRecording ? 'bg-red-600 hover:bg-red-700 border-red-400 animate-pulse-red shadow-lg shadow-red-900/40' : ''}`}
        >
          {isProcessing ? (
      ...
      {isRecording && 
        <div className="text-center w-full animate-fade-in">
            ...
            <div className="text-red-500 font-bold animate-pulse tabular-nums mb-2">
                Listening... {formatTime(timer)} / 01:00
            </div>
            <div className="text-indigo-300 text-sm transition-opacity duration-500 italic">
